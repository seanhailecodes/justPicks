...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Step 1: Find WNBA games that need resolution
    const now = new Date()
    ...
    console.log(`Found ${unresolvedGames.length} unresolved WNBA games`)

    // Step 2: Fetch scores from The Odds API
    const url = `https://api.the-odds-api.com/v4/sports/basketball_wnba/scores/?apiKey=${API_KEY}&daysFrom=3`

    console.log('Fetching WNBA scores from The Odds API...')
    ...
    for (const game of unresolvedGames) {
      ...
      for (const pick of picks || []) {
        ...
        if (!pickUpdateError) {
          ...
          const notifTitle = pickedTeam ? `${emoji} ${pickedTeam}` : `WNBA Pick ${emoji}`
