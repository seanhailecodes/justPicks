 ...
 * e.g. "Canelo Alvarez" → "canelo-alvar"
 */
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Fetch Boxing fight odds. Each event is a fight between two fighters.
    // We only need h2h (who wins the fight) — same model as UFC picks.
    const oddsResponse = await fetch(
      `https://api.the-odds-api.com/v4/sports/boxing_boxing/odds/?apiKey=${ODDS_API_KEY}&regions=us&markets=h2h&oddsFormat=american&bookmakers=draftkings,fanduel`
    );
    ...
    console.log(`Fetched ${oddsData.length} Boxing fights. Requests remaining: ${remainingRequests}`);
    ...
    const games = oddsData.map((event: any) => {
      ...
      const gameId = `boxing_${dateStr}_${slugify(awayFighter)}_vs_${slugify(homeFighter)}`;

      // Repurpose spread columns to show h2h moneyline odds on the pick buttons.
      // Boxing has no traditional point spread — users just pick who wins the fight.
      ...
      return {
        ...
        // No team codes for Boxing — display full fighter name (displayMode: 'fighter')
        home_team_code: null,
        away_team_code: null,
        home_team_logo: null,
        away_team_logo: null,
        ...
        locked: isStarted,
        season: seasonForDate(new Date(event.commence_time), "calendar"),
    ...
    console.error("fetch-boxing-games error:", error);
    return new Response(
